...
Investors that buy the Senior BONDs will have a guaranteed yield for the life of the BOND, which will be one of the 2 main properties of the BOND (along with TVL).

The liquidity provided by the "juniors'' and the value locked in the BONDs (when bought by seniors) are invested in various DeFi lending platforms and the resulting rewards cover the guaranteed rate for senior BONDs. Some of these rewards come in the form of governance tokens which our system will sell on Uniswap for the native token of our pool. We will consider Compound as our lending platform and DAI as our pool token.

Juniors will benefit from the extra rewards generated by DAIs locked in BOND by seniors in situations where the variable APY of Compound (including the COMP rewards) are higher than the weighted average guaranteed yields of current BONDs. On the other hand, in the event of falling rewards from Compound, the returns of Juniors are diminished and if need be, their locked funds will be used to pay for the guaranteed returns of BONDs. 
...
To do that, we have to be able to be able to calculate the profits and losses of the pool very efficiently. We do that by averaging all existing BONDs into one weighted average BOND with the following properties:
...
#### old sBOND sold after maturity date, 
...
The aggregate BOND (ABOND) represents the current Senior pool and it will help us calculate the health of the Senior pool at an instant. If the rewards generated by the Senior pool so far exceed the guaranteed rewards at this time (time factor included), the extra reward can be considered profit for the junior pool (and loss if it's negative).

To keep track of profits and losses of Junior Token holders (bbcDAI, if the provider is compound), we update the exchange rate of bbcDAI to DAI before certain events (juniorDeposit, juniorWithdraw, buyBond, endBond). 
...
When a BOND is created, a fixed guaranteed yield is calculated for it from on-chain data. The fixed rate is based on the moving average APY of the pool's provider, in this example Compound. We track the NET APY of Compound in a Smart Contract and calculate its moving average (or Kalman filter, TBD). In the same SC, we also track the price of COMP tokens that affect the NET APY.
...
- Add principal to aggregate BOND
- Add guaranteed reward to aggregate BOND
- Recalculate aggregate BOND weighted start and end
- Allocate the BOND the the Senior
- Exchange Senior deposit DAI to cDAI
- recalculate bbcDAI_to_DAI_ratio
- senior_deposits += amount

## Senior withdraw (end BOND)

Steps:
- Require current_timestamp > BOND.end_timestamp
- Require BOND.owner == Senior
- Exchange all comp on Uniswap and add cDAI to pool
- Substract BOND.principal from aggregate BOND
- Substract BOND.reward from aggregate BOND
- Recalculate aggregate BOND weighted start and end (with BOND removed)
- Exchange enough cDAI to pay the BOND's initial DAI + guaranteed reward
- Transfer the DAI to senior
- Delete BOND
- recalculate bbcDAI_to_DAI_ratio
- senior_deposits -= BOND.principal
