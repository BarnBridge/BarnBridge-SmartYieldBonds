...
import { withCompoundRate, bondSlippage, toBNj, e18, ERROR_MARGIN_ACCEPTABLE } from '@testhelp/index';
...
const fixture = async (wallets: Wallet[]) => {
  ...
  const seniorModel = (await deployContract(deployerSign, SeniorBondSlippageV1Artefact, [])) as SeniorBondSlippageExpV1;

  const pool = (await deployContract(<Wallet>deployerSign, SmartYieldPoolMockArtefact, [ctoken.address])) as SmartYieldPoolMock;
