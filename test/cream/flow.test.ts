...
const dumpState = (crToken: ICrCToken, controller: CreamController, smartYield: SmartYield, pool: CreamProvider, oracle: YieldOracle, granularity: number, underlying: ERC20) => {
  return async () => {

    const [creamSupplyRatePerBlock, spotDailySupplyRate, spotDailyDistributionRate, spotDailyRate, maxRatePerDay, oracleRatePerDay, underlyingBalance, underlyingFees, providerRatePerDay, maxBondDailyRate] = await Promise.all([
...
const fixture = () => {
  return async (wallets: Wallet[]) => {
    ...
    return {
      ...
      dumpState: dumpState(crToken, controller, smartYield, pool, oracle, oracleCONF.granularity, underlying),
      moveTimeWindowAndUpdate: moveTimeWindowAndUpdate(oracle),
    };
  };
};

describe('CREAM flow tests', async function () {

  it('yield and price movements', async function () {

    const { whaleSign, crToken, deployerSign, rewardsCollector, buyTokens, buyBond, sellTokens, redeemBond, redeemJuniorBond, dumpState, controller, moveTimeWindowAndUpdate, buyJuniorBond, smartYield } = await bbFixtures(fixture());
  ...
  it('CREAM switch controller', async function () {
