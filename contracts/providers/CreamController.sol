...
contract CreamController is IController, ICreamCumulator, IYieldOraclelizable {
      ...
      IController()
    {
      ...
      underlyingDecimals = ERC20(ICToken(CreamProvider(pool).cToken()).underlying()).decimals();
    ...
    returns (uint256 compGot, uint256 underlyingHarvestReward)
    {
        ...
        require(
          ...
          "CrC: harvest poolShare"
        );

        // deposit pool reward share with liquidity provider
        CreamProvider(pool)._takeUnderlying(address(this), poolShare);
        CreamProvider(pool)._depositProvider(poolShare, 0);
      ...
      public view returns (uint256)
    {
      ...
      return ICToken(CreamProvider(pool).cToken()).supplyRatePerBlock().mul(BLOCKS_PER_DAY);
    }
