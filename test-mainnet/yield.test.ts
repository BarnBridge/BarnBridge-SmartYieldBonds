 ...
 const fixture = () => {
  return async (wallets: Wallet[]) => {
    ...
    const [controller, seniorBond, juniorBond] = await Promise.all([
      deployCompoundController(deployerSign, pool, smartYield, uniswapPath),
    ...
    const [oracle ] = await Promise.all([
      deployYieldOracle(deployerSign, controller, oracleCONF.windowSize, oracleCONF.granularity),
